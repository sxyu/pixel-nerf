...
def extra_args(parser):
    parser.add_argument(
        "--input", "-I", type=str, default=os.path.join(ROOT_DIR, "input"), help="Image directory"
    )
    parser.add_argument(
        "--output", "-O", type=str, default=os.path.join(ROOT_DIR, "output"), help="Output directory"
    )
    ...
    parser.add_argument(
        "--num_views", type=int, default=24, help="Number of video frames (rotated views)"
    )
...
renderer = NeRFRenderer.from_conf(conf["renderer"], True).to(device=device)
...
inputs = [os.path.join(args.input, x) for x in inputs_all if x.endswith('_normalize.png')]
...
with torch.no_grad():
    for i, image_path in enumerate(inputs):
        ...
        frames = (rgb_fine.view(args.num_views, H, W, 3).cpu().numpy() * 255).astype(np.uint8)
        ...
        if not args.no_vid:
            if args.gif:
                ...
                imageio.mimwrite(
                    vid_path, frames, fps=args.fps
                )
            else:
                vid_path = os.path.join(args.output, im_name + "_vid.mp4")
                imageio.mimwrite(
                    vid_path, frames, fps=args.fps, quality=8
                )
