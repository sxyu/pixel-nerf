...
POINTREND_ROOT_PATH = osp.join(ROOT_PATH, 'detectron2', 'projects',
                                   'PointRend')
...
if not os.path.exists(POINTREND_ROOT_PATH):
    ...
    urllib.request.urlretrieve("https://alexyu.net/data/pointrend_min.zip", zipfile_name)
...
try:
    ...
    print("Please install Detectron2 by selecting the right version",
...
def _is_image_path(f):
    return f.endswith('.jpg') or f.endswith('.jpeg') or f.endswith('.png') or f.endswith('.bmp') or \
           f.endswith('.tiff') or f.endswith('.gif')
...
if __name__ == '__main__':
    ...
    for image_path in tqdm.tqdm(input_images):
        ...
        if len(masks) == 0:
            print("WARNING: PointRend detected no objects in", image_path,
                  "skipping")
            continue
        mask_main = masks[0]
        ...
        masked_crop = cv2.resize(masked_crop, (args.size, args.size),
                interpolation=cv2.INTER_LINEAR)

        if len(mask_crop.nonzero()[0]) == 0:
            print("WARNING: cropped mask is empty for", image_path,
                  "skipping")
            continue
        ...
        out_masked_path = os.path.join(INPUT_DIR,
                                     img_no_ext + "_normalize.png")
