...
def extra_args(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="1",
        help="Number of source views (multiview PIFu); put multiple (space delim) to pick randomly per batch",
    )
...
class ShapenetTrainer(trainlib.Trainer):
        ...
        if args.resume:
            ...
            if os.path.exists(self.alpha_prior_state_path):
                self.crit_alpha_prior.load_state_dict(
                    torch.load(self.alpha_prior_state_path)
                )
    ...
    def calc_losses(self, data, is_train=True):
        ...
        if "bbox" in data:
            all_bboxes = data["bbox"]  # (SB, NV, 4)  cmin rmin cmax rmax
        else:
            all_bboxes = None
        ...
        for obj_idx in range(SB):
            ...
            if curr_nviews > 1:
                ...
                image_ord[obj_idx] = torch.randperm(NV)[:curr_nviews]
    ...
    def vis_step(self, data, idx=None):
        ...
        cam_rays = util.gen_rays(
            poses, W, H, focal, self.z_near, self.z_far
        )  # (NV, H, W, 8)
        images_0to1 = images * 0.5 + 0.5  # (NV, 3, H, W)

        curr_nviews = nviews[torch.randint(0, len(nviews), (1,)).item()]
        views_src = torch.randperm(NV)[:curr_nviews]
        ...
        with torch.no_grad():
            ...
            if len(fine) > 0:
                warnings.warn(
                    "Fine returned outputs but we don't want to use them now. Check your config"
                )
