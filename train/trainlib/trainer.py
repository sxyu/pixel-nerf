...
class Trainer:
    def __init__(self, net, train_data_loader, test_data_loader, args, conf):
        self.args = args
        self.net = net
        ...
        if args.resume:
            if os.path.exists(self.optim_state_path):
                self.optim.load_state_dict(torch.load(self.optim_state_path))
            if self.lr_scheduler is not None and os.path.exists(
                self.lrsched_state_path
            ):
                self.lr_scheduler.load_state_dict(torch.load(self.lrsched_state_path))
            if os.path.exists(self.iter_state_path):
                self.start_iter_id = torch.load(self.iter_state_path)["iter"]
    ...
    def start(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch, data in tqdm.tqdm(enumerate(self.train_data_loader)):
